...
.pg-bar{display:flex;align-items:center;justify-content:center;gap:14px;margin-top:14px}.pg-btn{border:1px solid var(--line);background:#fff;border-radius:9px;padding:7px16px;font-weight:700;font-size:12px;cursor:pointer;color:var(--ink)}.pg-btn:hover:not(:disabled){border-color:var(--blue);color:var(--blue)}.pg-btn:disabled{opacity:.4;cursor:default}.pg-info{color:var(--muted);font-size:12px;font-variant-numeric:tabular-nums}
...
const PAGE_SIZE =8;
const pageState = {};
function paginate(key, items, renderPage, target, emptyText){
 if(!items.length){target.innerHTML='<div class="empty">'+emptyText+'</div>';return}
 const ps = PAGE_SIZE;
 if(!pageState[key] || pageState[key] >= Math.ceil(items.length/ps)) pageState[key]=0;
...
function renderLiveV2(d){
 const live=d.live_v2||{}, runtime=live.runtime||{}, limits=live.limits||{}, t=live.totals||{}, detail=(runtime&&runtime.detail)||{}, orders=live.orders||[];
 ...
 const actHtml=act.length?act.map(x=>{const d=x.detail||{};const reason=d.error||d.reason||(x.decision_id!=null?'decision '+x.decision_id:'');return `<div class="event"><div class="event-time">${dateTime(x.created_at_utc)}</div><div><div class="event-title"><span class="status-icon"></span>${esc(x.from_state||'start')} → ${esc(x.to_state)}</div><div class="event-detail">${esc(String(reason).slice(0,80))}</div></div></div>`}).join(""):'<div class="empty">No activity yet.</div>';
 $('live-activity').innerHTML=actHtml;
 ...
 <div class="panel-head" style="margin-top:14px"><div><h2>Activity log</h2><div class="panel-note">what the bot is doing right now · journal transitions · Almaty time</div></div></div>
 <div id="live-activity" class="timeline"></div>`;
...
function renderWeatherNextPaper(d){
 ...
 const cards=[['Realized P&L',money(p.realized_pnl_usd),pnl>=0?'good':'negative'],['Open exposure',money(p.open_exposure_usd),'neutral'],['Settled',p.settled_orders||0,'neutral'],['Decisions',Object.values(p.decisions_by_action||{}).reduce((a,b)=>a+Number(b||0),0),'neutral']];
...
function renderTimeline(d){
 ...
 const html=slice=>slice.map(r=>`<div class="event"><div class="event-time">${dateTime(r.created_at)}<br><small>${elapsed(r.elapsed_seconds)}</small></div><div><div class="event-title"><span class="status-icon"></span>${esc(r.kind.replaceAll('_',' '))}</div><div class="event-detail">${r.payload?.error?esc(r.payload.error):r.payload?.scan?`${r.payload.scan.events_scanned||0} events · ${r.payload.scan.markets_scanned||0} markets · ${r.payload.scan.paper_orders_opened||0} opened`:esc(r.payload?.message||'Autonomous service is running.')}</div></div></div>`).join('');
 paginate('timeline',reports,html,$('timeline'),'Reports will appear automatically.');
}
function renderDecisions(d){
 const decisions=d.decisions||[];
 if(!decisions.length){$('decisions').innerHTML='<div class="empty">No recent decisions.</div>';return}
 const interesting=decisions.filter(x=>x.action!=='SKIP'||(x.expected_profit_usd!=null&&Number(x.expected_profit_usd)>0));
 const source=(interesting.length?interesting:decisions);
 const html=slice=>slice.map(x=>`<div class="decision"><div class="decision-top"><span class="decision-id">${esc(x.market_id)}</span><span class="pill ${x.action==='PAPER_BUY'?'green':x.action==='OBSERVE'?'blue':'gray'}">${esc(x.action)}</span></div><div class="decision-metrics"><span>p ${x.probability==null?'—':(Number(x.probability)*100).toFixed(1)+'%'}</span><span>EV ${money(x.expected_profit_usd)}</span><span>entry ${money(x.executable_price)}</span></div><div class="sub">${dateTime(x.created_at)}</div><div class="decision-reason">${esc((x.reason_codes||[]).concat(x.warning_codes||[]).join(' · ')||'qualified')}</div></div>`).join('');
 paginate('decisions',source,html,$('decisions'),'No recent decisions.');
}
...
refresh();refreshComparison();setInterval(refresh,8000);setInterval(refreshComparison,60000);
</script>
</body>
</html>"""
