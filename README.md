...
`polybot.live_pilot` — узкий аварийно-безопасный контур для будущего пилота:

- только один `BUY`, только `FOK`: notional максимум `$1.90`, а signed
  `max_spend` с консервативной комиссией — максимум `$2.00`;
- отдельный кошелёк с collateral-балансом не более `$10.00`;
- точная привязка разрешения к SHA-256 intent, token/condition/market,
  `book_hash`, цене и сроку действия;
- SQLite-журнал intent и подписанного payload записывается **до** POST;
- timeout/потерянный ответ помечается `AMBIGUOUS` и никогда слепо не повторяется;
- после сбоя сверяются account trades и open orders; неясный результат требует
  ручной проверки;
- недостаточный allowance завершается fail-closed: библиотека не вызывает
...
Новая доказанно неподписанная предпроверка не расходует дневную попытку:
граница подписи журналируется отдельно. Старые записи при обновлении не сбрасываются.
...
### Офлайн-симулятор денежного риска

`polybot.offline_risk_sim` моделирует только paper-состояния и не импортирует
live-исполнитель, SDK, ключи или сетевой клиент. По умолчанию он фиксирует:

- `$2` максимального риска позиции и `$2` суммы реализованных дневных убытков
  плюс незакрытого худшего риска;
- одну позицию одновременно и фиксированный исследовательский капитал `$10`;
  совокупные реализованные убытки уменьшают доступный капитал навсегда в
  рамках симуляции, а прибыль его не восстанавливает;
- сутки `Asia/Almaty`, при этом незакрытая позиция переносит весь риск в новый
  день;
- прибыль не восстанавливает ни дневной лимит убытка, ни уже использованный
  капитал;
- доказанный отказ до отправки освобождает резерв, а неоднозначный результат
  остаётся на карантине до явной сверки;
- повтор одного и того же перехода не создаёт второй резерв или второй убыток.

JSON-сценарий можно воспроизвести локально:

```bash
.venv/bin/python scripts/simulate-cash-risk.py path/to/scenario.json
```

Результат всегда помечен `OFFLINE_PAPER_RISK_SIMULATION` и не отражает баланс
кошелька или состояние production.
