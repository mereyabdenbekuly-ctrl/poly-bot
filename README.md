...
> года минимальный one-shot live-pilot реализован только как отключённая
> библиотека: у него нет CLI, таймера, systemd-сервиса, ключа или связи со
> scanner. До отдельного подтверждения точного intent он ничего не подписывает
> и не отправляет.

### Отключённый live-pilot
...
Это не команда для запуска. Для намеренного отделения от наблюдателя отсутствуют
CLI, cron/systemd unit и переменные ключа в `.env.example`. Приватный ключ нельзя
передавать в чат, git, `.env` или аргументах процесса; подключение кошелька
выполняется отдельной операторской процедурой только после нового разрешения.
